...
#+begin_src lisp
  ;; Only fire when light changes TO "true"
  (defrule "Light turned on"
    :when-item-transition '(my-light :to "true")
    :do (lambda (trigger)
          (let* ((event (cdr trigger))
                 (old-value (item:item-changed-event-old-value event)))
            (format t "Light turned on, was: ~a~%" old-value))))

  ;; Fire when sensor changes FROM "off" TO "on"
  (defrule "Sensor activated"
    :when-item-transition '(my-sensor :from "off" :to "on")
    :do (lambda (trigger)
