...
- a UI to interact with the system (rudimentary and usable aaand WIP) ([[https://github.com/mdbergmann/chipi/blob/main/README-web.md#chipi-ui][details]])
          ...
          (case (car trigger)
            (:item
             (let* ((event (cdr trigger))
               ...
               (format t "Item changed: ~a, old: ~a~%" item old-value)
...
For item triggers, the =cdr= of the trigger is an =item:item-changed-event= struct. Use =item:item-changed-event-item= to get the item and =item:item-changed-event-old-value= to get the value before the change.

Rules can also be triggered conditionally on specific value transitions using =:when-item-transition=. This allows firing only when an item changes FROM and/or TO specific values:

#+begin_src lisp
  ;; Fire when light is turned on
  (defrule "Light turned on"
    :when-item-transition '(my-light :from item:false :to item:true)
    :do (lambda (trigger)
          (let* ((event (cdr trigger))
                 (old-value (item:item-changed-event-old-value event)))
            (format t "Light turned on, was: ~a~%" old-value))))

  ;; Fire when light is turned off (any previous value)
  (defrule "Light turned off"
    :when-item-transition '(my-light :to item:false)
    :do (lambda (trigger)
          (declare (ignore trigger))
          (do-some-action)))

  ;; Fire on any change away from "standby"
  (defrule "Left standby"
    :when-item-transition '(my-sensor :from "standby")
    :do (lambda (trigger)
          (declare (ignore trigger))
          (do-some-action)))
#+end_src

Multiple =:when-item-transition= entries can be specified per rule. If both =:when-item-change= and =:when-item-transition= match the same item change, the =:do= function fires only once (transition match takes precedence).
...
Chipi provides an optional event log that records all item value changes to an NDJSON (newline-delimited JSON) file. This is useful for debugging, monitoring, and auditing item state changes.
...
To enable, call =ensure-event-log= inside your =defconfig= and register the shutdown hook:

#+begin_src lisp
  (defconfig "my-system"
    (event-log:ensure-event-log :max-file-size (* 5 1024 1024)
                                 :max-backup-files 3)
...
The log is written to =runtime/event-log/event-log.ndjson=. When the file exceeds =:max-file-size= (default 10 MB), it is rotated (=.1=, =.2=, ..., up to =:max-backup-files=, default 5). All parameters are optional.

If you don't call =ensure-event-log=, no event log is created and there is no overhead.
