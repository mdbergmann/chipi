...
The project uses log4cl. You can change the log level for '=(cl-hab)= to suite your needs of granularity. The underlying Sento actor framework will log extensively on =:debug= level so a good idea is to silence this via =(log:config '(sento) :warn)=.
